...
> **Ledger ownership (hard rule).** `verification.json` is checker-owned: the LLM only **reads** it to decide what to re-verify, never edits it by hand. Full rule: `checker-spec.md` §6.
