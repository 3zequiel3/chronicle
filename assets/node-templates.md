...
The templates above are the `web_app` form. For a **non-web** `system_type` (`cli`, `library_sdk`, `data_pipeline`) the variant forms for the reframed slots live in **`node-templates-profiles.md`**, loaded **only** for those profiles (token economy — web/api/mobile/saas never load it). `api`, `mobile` and `saas_multi_tenant` use the base templates above with the profile-table adjustments and need no variant file.
